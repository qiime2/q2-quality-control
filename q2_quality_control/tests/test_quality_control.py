...
class TestIdentify(TestPluginBase):
    ...
    def test_prevalence(self):
        ...
        output_feature_table = decontam_identify(asv_or_otu_table=
                                                 self.asv_table,
                                                 meta_data=
                                                 self.metadata_input,
                                                 decon_method=
                                                 ...
                                                 ='Control')
        df_output_feature_table = transform(output_feature_table,
...
class TestRemove(TestPluginBase):
    ...
    def test_remove(self):
        exp_table = pd.read_csv(
            ...
            sep='\t', index_col=0)
        output_asv_table = decontam_remove(asv_or_otu_table=self.asv_table,
